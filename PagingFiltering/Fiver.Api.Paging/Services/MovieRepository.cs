...
namespace Movies.API.Services
{
    public class MovieRepository : IMovieRepository
    {
        ...
        {
            return new PagedList<Movie>(this.movies, pagingParams.PageNumber, pagingParams.PageSize);
        }

        public PagedList<Movie> GetMovies(PagingParams pagingParams, string search)
        {
            if (!string.IsNullOrEmpty(search))
            {
                search = search.Trim().ToLower();
                {
                    this.movies = this.movies
                           .Where(m => (bool)(m.Title.ToLower().Contains((string)search)
                                || m.Summary.ToLower().Contains((string)search))).ToList();
                }
            }
        ...
        #region Filter
        public PagedList<Movie> GetMovies(PagingParams pagingParams, string title = null, string releaseYear = null, string summary = null)
        {
            IEnumerable<Movie> ret = this.movies;
            if (!string.IsNullOrEmpty(title))
            {
                ret = ret.Where(s => s.Title.Contains(title, StringComparison.CurrentCultureIgnoreCase));
            }
            if (!string.IsNullOrEmpty(summary))
            {
                ret = ret.Where(s => s.Summary.Contains(summary, StringComparison.CurrentCultureIgnoreCase));
            }
            if (!string.IsNullOrEmpty(releaseYear))
            {
                ret = ret.Where(s => s.ReleaseYear == int.Parse(releaseYear));
            }
            return new PagedList<Movie>(ret, pagingParams.PageNumber, pagingParams.PageSize);
        }

        public PagedList<Movie> GetMovies(PagingParams pagingParams, IEnumerable<Filter> filters)
        {
            IEnumerable<Movie> ret = this.movies;
            foreach(var filter in filters)
            {
                switch(filter.Attribute.ToLower())
                {
                    case "title":
                        ret = TitleFilter(ret, filter);
                        break;
                    case "releaseyear":
                        ret = ReleaseYearFilter(ret, filter);
                        break;
                    case "summary":
                        ret = SummaryFilter(ret, filter);
                        break;
                    default:
                        break;
                }
        ...
        public IEnumerable<Movie> TitleFilter(IEnumerable<Movie> movies, Filter filter)
        {
            var values = filter.Values as List<string>;
            switch(filter.Operator.ToLower())
            {
                case "in":
                    foreach(var val in values)
                    {
                        movies = movies.Where(s => s.Title.Contains(val, StringComparison.CurrentCultureIgnoreCase));
                    }
                    break;
                case "not in":
                    foreach (var val in values)
                    ...
                    break;
                case "empty":
                    movies = movies.Where(s => s.Title == string.Empty);
                    break;
                case "not empty":
                    movies = movies.Where(s => s.Title != string.Empty);
                    break;
        ...
        public IEnumerable<Movie> SummaryFilter(IEnumerable<Movie> movies, Filter filter)
        {
            var values = filter.Values as List<string>;
            switch (filter.Operator.ToLower())
            {
                case "in":
                    foreach (var val in values)
                    {
                        movies = movies.Where(s => s.Summary.Contains(val, StringComparison.CurrentCultureIgnoreCase));
                    }
                    break;
                case "not in":
                    foreach (var val in values)
                    ...
                    break;
                case "empty":
                    movies = movies.Where(s => s.Summary == string.Empty);
                    break;
                case "not empty":
                    movies = movies.Where(s => s.Summary != string.Empty);
                    break;
        ...
        public IEnumerable<Movie> ReleaseYearFilter(IEnumerable<Movie> movies, Filter filter)
        {
            var values = filter.Values as List<string>;
            if (!values.Any()) return movies;

            switch (filter.Operator.ToLower())
            {
                case "<":
                        movies = movies.Where(s => s.ReleaseYear < int.Parse(values[0]));
                    break;
                case "<=":
                        movies = movies.Where(s => s.ReleaseYear <= int.Parse(values[0]));
                    break;
        ...
        private void GenerateDataModel()
        {
            this.movies = new List<Movie>
            {
                ...
                new Movie { Id = 22, Title = "", ReleaseYear = 2015, Summary = "A cryptic message from Bond's past sends him on a trail to uncover a sinister organization. While M battles political forces to keep the secret service alive, Bond peels back the layers of deceit to reveal the terrible truth behind SPECTRE." },
            };
        }
    }
}
